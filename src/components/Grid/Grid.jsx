...
const Grid = forwardRef(({ playerCount = 2 }, ref) => {
    const [scores, setScores] = useState(() => {
        return ROUNDS.map(() => Array(playerCount).fill({ select1: '-', select2: '-' }));
    });
    ...
    useEffect(() => {
        setScores(() => {
            return ROUNDS.map(() =>
                Array.from({ length: playerCount }, () => ({ select1: '-', select2: '-' }))
            );
        });
    ...
    const renderPlayerName = (index) => {
        ...
        return (
            <span onClick={() => handleNameClick(index)} className={styles.playerName}>
                {playerNames[index]}
            </span>
        );
    };
    ...
    const handleScoreChange = (roundIndex, playerIndex, selectIndex, value) => {
        if (value === '0' && !isValidSelection(roundIndex, playerIndex, selectIndex)) {
            toast.error('A player cannot select 0 three times in a row.');
            return;
        }

        if (selectIndex === 1 && !isValidSum(roundIndex, playerIndex, value)) {
            toast.error(`Player ${playerIndex + 1} cannot select this number as it would make the sum equal to the round number.`);
            return;
        }
    ...
    const isValidSelection = (roundIndex, playerIndex, selectIndex) => {
        ...
        const prevRound1 = scores[roundIndex - 1]?.[playerIndex]?.[`select${selectIndex}`];
        const prevRound2 = scores[roundIndex - 2]?.[playerIndex]?.[`select${selectIndex}`];
    ...
    const renderSelect = (roundIndex, playerIndex, selectIndex) => {
        ...
        const options = ['-', ...Array.from({ length: ROUNDS[roundIndex] + 1 }, (_, i) => i)];
        return (
            <select
                value={scores[roundIndex][playerIndex][`select${selectIndex}`]}
                className={styles.selectInput}
                onChange={(e) => handleScoreChange(roundIndex, playerIndex, selectIndex, e.target.value)}
            >
                {options.map((num) => (
    ...
    return (
        <div className={styles.gridWrapper}>
            <div className={styles.gridContainer} style={{ '--player-count': playerCount }}>
                ...
                {ROUNDS.map((roundNum, roundIndex) => (
                    <div key={roundIndex} className={styles.row}>
                        ...
                        {Array.from({ length: playerCount }).map((_, playerIndex) => (
                            <div key={`cell-${roundIndex}-${playerIndex}`} className={styles.splitCell}>
